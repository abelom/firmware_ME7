...
static void setLedPins() {
	...
	engine->communicationLedPin = EFI_COMMUNICATION_PIN;
#else
	engine->communicationLedPin = GPIOE_2; // d23 = blue
#endif /* EFI_COMMUNICATION_PIN */
	engine->runningLedPin = GPIOE_4;		// d22 = green
	engine->triggerErrorPin = GPIOE_1;		// d27 = orange
}
...
static void setupTle8888() {
	...
	// Wire up spi3
	engine->spi3mosiPin = GPIOB_5;
	engine->spi3misoPin = GPIOB_4;
	engine->spi3sckPin = GPIOB_3;

	// Chip select
	engine->tle8888_cs = GPIOD_5;

	// SPI device
	engine->tle8888spiDevice = SPI_DEVICE_3;
}
...
static void setupEtb() {
	...
	// PWM pin
	engine->etbIo[0].controlPin1 = GPIOC_7;
	// DIR pin
	engine->etbIo[0].directionPin1 = GPIOA_8;
	// Disable pin
	engine->etbIo[0].disablePin = GPIOC_8;
	// Unused
...
void setSerialConfigurationOverrides(void) {
	engine->useSerialPort = false;
	engine->binarySerialTxPin = GPIO_UNASSIGNED;
	engine->binarySerialRxPin = GPIO_UNASSIGNED;
	engine->consoleSerialTxPin = GPIO_UNASSIGNED;
	engine->consoleSerialRxPin = GPIO_UNASSIGNED;
}
